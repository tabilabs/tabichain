...
for running end-to-end tests on Tabi.
It is used for testing chain upgrades,
as it allows for initializing multiple Tabi chains with different versions.
...
# version(s) of initial tabi node(s) that will be upgraded, tag e.g. 'v9.1.0'
# to use multiple upgrades separate the versions with a forward slash, e.g.
# 'v10.0.1/v11.0.0-rc1'
INITIAL_VERSION

# version of upgraded tabi node that will replace the initial node, tag e.g.
# 'v10.0.0'
TARGET_VERSION

# mount point for the upgraded node container, to mount new node version to
# previous node state folder. By default this is './build/.tabid:/root/.tabid'
# More info at https://docs.docker.com/engine/reference/builder/#volume
MOUNT_PATH

# '--chain-id' tabi cli parameter, used to start nodes with a specific
# chain-id and submit proposals
# By default this is 'tabi_9000-1'
CHAIN_ID
```
...
1. Build a docker image for the tabi target version
...
the test suite exports `/.tabid` folder from the docker container
...
This package is decoupled from depending on the Tabi codebase.
It initializes the chains for testing via Docker.  
As a result, the test suite may provide the
desired Tabi version to Docker containers during the initialization.
This design allows for the opportunity of testing chain upgrades
by providing an older Tabi version to the container,
...
to run different versions of tabi containers,
...
will be pulled from [DockerHub](https://hub.docker.com/r/tharsishq/tabi/tags).
If it is not specified,
the test suite retrieves the second-to-last upgrade version
from the local codebase (in the `tabi/app/upgrades` folder)
...
```log
ok  	github.com/tabi/tabi/v9/tests/e2e	174.137s.
```
...
Error:  Received unexpected error:
        can't start tabi node, container exit code: 2
        ...
        main.appCreator.newApp({{{0x3399b40, 0xc000ec1db8}, {0x33ac0f8, 0xc00...
            github.com/tabi/tabi/v10/cmd/tabid/root.go:243 +0x2ca
        ...
        main.main()
            github.com/tabi/tabi/v10/cmd/tabid/main.go:20 +0x45
...
```log
CONTAINER ID   IMAGE
9307f5485323   tabi:local    <-- upgraded node
f41c97d6ca21   tabi:v9.0.0   <-- initial node
```
